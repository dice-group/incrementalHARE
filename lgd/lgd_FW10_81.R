...
t1=proc.time()
	...
	starting_ch0=71
	if(starting_ch0 > 1){#continue to a previous Run
		 print(sprintf("skipping # Chunks:%d",starting_ch0-1))
		 aa=scan(con_s,what=list(NULL),sep='\n',blank.lines.skip = F,skip=chnkLen*(starting_ch0-1)-1,nlines=1)
	...
	#st_l=chnkLen*(starting_ch0-1) - .Machine$integer.max
...
#		tmp2 <- foreach(i = 1:ceiling(NChnks/NCH),.packages='Matrix', .combine="c") %dopar% {
    for(i in ceiling(starting_ch0/NCH):ceiling(NChnks/NCH)){
            print(i)
